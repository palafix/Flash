...
const val NOTIF_CHANNEL_DOWNLOADS = "downloads"// The id of the downloads channel in Utils.kt.
...
fun setupNotificationChannels(c: Context) {
    ...
    val ntf = c.string(R.string.notifications)
    val dls = c.string(R.string.download)
    manager.notificationChannels
            .filter {
                it.id != NOTIF_CHANNEL_GENERAL
                        && it.id != NOTIF_CHANNEL_MESSAGES
                                && it.id != NOTIF_CHANNEL_DOWNLOADS
            }
            .forEach { manager.deleteNotificationChannel(it.id) }
    manager.createNotificationChannel(NOTIF_CHANNEL_GENERAL, "$appName: $ntf")
    manager.createNotificationChannel(NOTIF_CHANNEL_MESSAGES, "$appName: $msg")
    manager.createNotificationChannel(NOTIF_CHANNEL_DOWNLOADS, "$appName: $dls")
...
private fun NotificationManager.createNotificationChannel(id: String, name: String): NotificationChannel {
    ...
    channel.lightColor = R.color.flash_notification_icon
