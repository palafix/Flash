 ...
 *
 */
object MessageParser : FlashParser<FlashMessages> by MessageParserImpl() {

    fun queryUser(cookie: String?, name: String) = parseFromUrl(cookie, "${FbItem.MESSAGES.url}/?q=$name")

}

data class FlashMessages(val threads: List<FlashThread>,
                         val seeMore: FlashLink?,
                         val extraLinks: List<FlashLink>
) : ParseNotification {
    override fun toString() = StringBuilder().apply {
        append("FlashMessages {\n")
 ...
 */
data class FlashThread(val id: Long,
                       val img: String?,
                       val title: String,
                       val time: Long,
                       val url: String,
...
private class MessageParserImpl : FlashParserBase<FlashMessages>(true) {
    ...
    override fun parseImpl(doc: Document): FlashMessages? {
        ...
        return FlashMessages(threads, seeMore, extraLinks)
    }

    private fun parseMessage(element: Element): FlashThread? {
